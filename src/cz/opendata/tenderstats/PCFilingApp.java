...
//		Resource contractRes = contract.getResource(contractURI);		
	...
	//ADMIN
	protected ResultSet actionTableOntologies(HttpServletRequest request, UserContext uc) {	
		return modelOntology.getOntologies(uc.getNamedGraph());		
	}
	
	//ADMIN
	protected ResultSet actionTableForms(HttpServletRequest request, UserContext uc) {	
		return modelOntology.getForms(uc.getNamedGraph());		
	}
	...
	protected void doGetPost(HttpServletRequest request,
		...
		if (isUserLoggedIn(request)) {
			...
			case "getContractURIsJson":
				...
				String rdf = modelContract.getPrivateContractURIsJson(contractURI,
						uc.getNamedGraph(), "all");

				response.setContentType("application/text; charset=UTF-8");
				response.getWriter().println(rdf);
			...
			case "getPublicContractJson":
				...
				JsonObject ret = modelContract.getPublicContractAsJson(model.getPublicContract(contractURL),contractURL); 
				json.add("data", ret);
				json.addProperty("success", true);
			...
			case "editEvent":
				...
				modelContract.deletePrivateContract(uc, contractURI);
				modelContract.addPrivateContract(getUserContext(request),
						request.getParameter("title"),
						request.getParameter("description"),
						request.getParameter("cpv1"),
						...
						request.getParameter("tendersSealed"), contractRes
								...
								.getLiteral().getString(), contractURI, request);
