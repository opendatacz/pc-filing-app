...
<html>
    ...
    <body>
        ...
        --%>
            ...
            <thead>
              <tr>
                  <th><fmt:message key="title" bundle="${cons}" /></th>
                                ...
                                <th><fmt:message key="submitted" bundle="${cons}" /></th>
		  ...
		  <ul>
				<li><a onclick="$('.3dots').remove(); $('#pages li').removeClass('reallyhide'); $('#showAllPages').remove(); " href="#"><fmt:message key="showallpages" bundle="${cons}" /></a>
				</li>
		  </ul>
		  </div>
  ...
  <%@include file="WEB-INF/jspf/footer.jspf" %> 
	...
	<script type="text/javascript">
		...
		var tableAddress = "PCFilingApp?action=table&tableName=SubmittedTenders";
		...
		function fillTable() {
			...
			$.each(					
					...
					function(i, data) {
						...
						// Created
						newCreated = $('<td>').append((data.deadline).substring(0,10)).appendTo(newRow);												
						...
						newActions.addClass('btn-group');
							...
							newView.click(function() { showTender(data.title,data.tenderURI); });
							newView.append('view').appendTo(newActions);
						
							// Withdraw
							newWithdraw = $('<a>');
							newWithdraw.attr('href','PCFilingApp?action=withdrawTender&forward=supplier-withdrawn.jsp&tenderURL=' + encodeURIComponent(data.tenderURI) + '&contractURL=' + encodeURIComponent(data.contractURI) + '&buyerURL=' + encodeURIComponent(data.buyerURI));
							newWithdraw.click(function() { return confirm('<fmt:message key="submitted.withdraw.confirm" />'); });
							newWithdraw.append('withdraw').appendTo(newActions);
