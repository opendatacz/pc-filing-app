  ...
  def sendPost(uri: String, graph: String) = {
    ...
    try {
      val m = Sparql
        ...
        .execConstruct
      m.getResource(uri).addProperty(ResourceFactory.createProperty(Config.cc.getPrefix("pc"), "kind"), "")
      m.write(baos, "JSONLD")
      val x = JsonUtils.toString(JsonLdProcessor.frame(JsonUtils.fromString(baos.toString("UTF-8")), jsonLdFrame, jsonLdOptions))
