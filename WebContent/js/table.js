...
function CPVs(cpv1, cpvAll) {
	...
	if (cpvAll != undefined) {
		var cpvsTemp = cpvAll.split(" ");
		for(var i = 0; i<cpvsTemp.length; i++){
			cpv.push(cpvsTemp[i].replace("http://purl.org/weso/cpv/2008/", "", "g"));		
		}
	}	
	var list = $("<ul>");	
...
//new function to get CPV codes as array
function CPVsInArray(cpv1, cpvAll){
	var cpv = new Array();
	if (cpv1 != undefined) {
		cpv.push(cpv1);						
	}
...
//new function to get specifications in array
function specsInArray(specs, uri){
	var specsArray = new Array();

	if (specs != undefined) {
		var specsTemp = specs.split(" ");
		for(var i = 0; i<specsTemp.length; i++){
			//specsArray.push(specsTemp[i].substring(uri.length+1, uri.length+9));
			specsArray.push(specsTemp[i]);
		}
	}
...
//new function to store ontologyURI into session
function editOntology(ontology) {
	sessionStorage.ontologyURI = ontology;
}
//new function to store classURI into session
function viewForm(formClass) {
	sessionStorage.formClass = formClass;
}
//new function to store specificationURI into session
function viewSpec(specURI) {
	sessionStorage.specURI = specURI;
}
...
//new function to get actual date and time
